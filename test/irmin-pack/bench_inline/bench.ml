...
type metrics = {
  ...
  write_time_ms : float;
  read_latencies_us : float array;
      (* Individual read latencies in microseconds *)
  inlined_count : int;
...
let reset_env ~fs () = Eio.Path.rmtree ~missing_ok:true (root fs)
let info () = S.Info.empty

let open_repo ~sw ~fs ~fresh ~readonly ~inline_contents () =
  let conf =
    Irmin_pack.Conf.init ~sw ~fs ~fresh ~readonly ~inline_contents (root fs)
  in
  S.Repo.v conf
...
let rec dir_size path =
  if Sys.is_directory path then
    ...
    Array.fold_left
      (fun acc entry -> acc + dir_size (Filename.concat path entry))
      0 entries
...
let run_one ~sw ~fs ~config =
  ...
  let repo =
    open_repo ~sw ~fs ~fresh:true ~readonly:false
      ~inline_contents:config.inline_contents ()
  in
  let main = S.main repo in
  ...
  let tree =
    Array.fold_left
      (fun tree (path, value, _size) -> S.Tree.add tree path value)
      (S.Tree.empty ()) contents
  in
  ...
  let repo =
    open_repo ~sw ~fs ~fresh:false ~readonly:true
      ~inline_contents:config.inline_contents ()
  in
  let main = S.main repo in
  let head = S.Head.get main in
  ...
  let read_latencies_us =
    Array.init config.n_reads (fun _ ->
        ...
        | Some v ->
            Printf.eprintf
              "Warning: value mismatch at %s: expected %d bytes, got %d bytes\n"
              (String.concat "/" path)
              (Bytes.length expected_value)
              (Bytes.length v)
        | None ->
...
let run ~fs ~config =
  ...
  let all_latencies =
    Array.concat
      (Array.to_list (Array.map (fun r -> r.read_latencies_us) results))
  in
  ...
  ( median_store_size,
    ...
    p999,
    r0.inlined_count,
    r0.non_inlined_count,
    r0.content_sizes )

(* Print results as CSV row *)
let print_csv_header () =
  Printf.printf
    "distribution,threshold,n_contents,store_bytes,write_ms,read_p50_us,read_p99_us,read_p999_us,inlined,non_inlined,size_stats\n"

let print_csv_row ~dist_name ~config
    (store_size, write_time, p50, p99, p999, inlined, non_inlined, sizes) =
  let threshold_str =
    if config.inline_contents then string_of_int config.inline_threshold
    else "off"
  in
  Printf.printf "%s,%s,%d,%d,%.2f,%.2f,%.2f,%.2f,%d,%d,\"%s\"\n" dist_name
    threshold_str config.n_contents store_size write_time p50 p99 p999 inlined
    non_inlined

(* Generator for version_3_always test data *)
(*
   To regenerate the test data:
   1. dune exec test/irmin-pack/gen_always.exe
   2. rm -rf test/irmin-pack/data/version_3_always
   3. mv version_3_always_new test/irmin-pack/data/version_3_always
...
let config ~sw ~fs root =
  Irmin_pack.config ~sw ~fs
    ~indexing_strategy:Irmin_pack.Indexing_strategy.always ~readonly:false
    ~fresh:true root
